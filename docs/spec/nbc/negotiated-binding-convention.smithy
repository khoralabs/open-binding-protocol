...
///
/// **N9. Row `created_seq` (commit metadata).** Implementations **MAY** persist a monotonic **`created_seq`** per stored graph row using **`khora.obp.nbc#NbcRowCommitMeta`** semantics. It is **not** part of **`khora.obp`** Smithy shapes (`docs/spec/model/shapes.smithy`); it supports NBC / adapter ordering and audit.
///
