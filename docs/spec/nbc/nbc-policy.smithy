...
/// First-commit ledger tick for a persisted **`khora.obp`** graph row (**`Party`**, **`Offer`**, **`Port`**, or an edge). **Not** a member of those shapes in `docs/spec/model/shapes.smithy`; **`ObpPersistence`** and NBC-aware adapters record **`created_seq`** (or equivalent) when they require monotonic row ordering or audit.
