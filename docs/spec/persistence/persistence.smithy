...
**Policy payloads:** **`bind_payload`** on **BindPort** / **ExtendOffer** inputs and **ListBinds** rows is **`Document`** on the **persistence** surface (storage projection), not part of the core graph shape; NBC defines **`NbcBindSatisfaction`** and **when** bind-policy validation runs (`docs/spec/nbc/nbc-policy.smithy`); concrete **`bind_policy`** JSON shapes are **product/host-defined**. **`max_bindings`**, **`terminal`**, bind-policy JSON, and TTL/expose context for ports live under **`khora.obp.nbc#NbcPortExposePolicy`**.

**Staging:** ports that must not be bindable yet are **not** EXPOSES'd (no separate lifecycle enum on **Port**).

**Orchestration reads:** **IsPortExposed**, **ListBinds**, **GetPortsSnapshot**, and **GetExtendingPartyId** mirror the **`ObpPersistence`** strategy surface in `@khoralabs/obp-core/persistence` (same semantics as TS **`OBPPersistenceClient`** helpers). NBC drivers use these reads when evaluating NBC preconditions.
...
**Decentralized session sync:** Normative protocol (checkpoints, Merkle tree, hashing, verification, fork semantics) is **`khora.obp.session#NegotiationSessionProtocol`** in `docs/spec/session/session-protocol.smithy`.

**Live negotiation frames:** Bilateral signed **Frame** DAG rules (transport-agnostic) are **`khora.obp.frame#NegotiationFrameProtocol`** in `docs/spec/frame/frame-protocol.smithy`. The HTTP/2 reference binding is **`khora.obp.frame.http2#Http2Binding`** in `docs/spec/transport/frame-binding-http2.smithy`.
""")
