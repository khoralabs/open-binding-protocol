...
**Transport confidentiality:** Internet hub-mediated deployments **MUST** use **`khora.obp.frame.mls`** (MLS per **RFC 9420**, outer envelope **`mls2`** per **`MlsHubEnvelope`**). Custodial deployments **MAY** use plaintext multiplex on the blob bus. Direct transports **MAY** rely on TLS only. See `docs/spec/frame/mls-hub-protocol.smithy`.
