...
export class SqliteObpPersistenceStrategy implements ObpPersistenceStrategy {
  ...
  constructor(
    ...
    private readonly validateBindPolicyAtExpose: ValidateBindPolicyAtExpose = () => {},
  ) {
...
export function createObpSqliteStrategy(
  db: Database,
  options?: { validateBindPolicyAtExpose?: ValidateBindPolicyAtExpose },
): SqliteObpPersistenceStrategy {
  return new SqliteObpPersistenceStrategy(db, options?.validateBindPolicyAtExpose);
}
