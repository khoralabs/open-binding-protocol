...
export const PAIRING_SECRET_ENVELOPE_MAGIC = "obp/frame-relay/pairing/v1" as const;
...
function normalizeAesKey(keyBytes: Uint8Array): Buffer {
  ...
  throw new FrameRelaySqliteError("pairing secret encryption key must be at least 32 bytes");
}
...
function parseEnvelopeJson(stored: string): PairingSecretEnvelopeV1 {
  ...
  try {
    ...
    throw new FrameRelaySqliteError("pairing secret envelope: invalid JSON after magic prefix");
  }
  if (parsed === null || typeof parsed !== "object" || Array.isArray(parsed)) {
    throw new FrameRelaySqliteError("pairing secret envelope: body must be a JSON object");
  }
  const rec = parsed as Record<string, unknown>;
  if (rec.v !== PAIRING_SECRET_ENVELOPE_V1) {
    throw new FrameRelaySqliteError("pairing secret envelope: unsupported version");
  }
  if (rec.alg !== PAIRING_SECRET_ENVELOPE_ALG) {
    throw new FrameRelaySqliteError("pairing secret envelope: unsupported algorithm");
  }
  if (typeof rec.iv !== "string" || typeof rec.ct !== "string") {
    throw new FrameRelaySqliteError("pairing secret envelope: iv/ct must be base64 strings");
  }
  return rec as PairingSecretEnvelopeV1;
}

/** AES-256-GCM envelope stored in `rooms.pairing_secret_hex` (column name is historical). */
...
export function decryptPairingSecretHex(stored: string, keyBytes: Uint8Array): string {
  ...
  if (iv.length !== 12) {
    throw new FrameRelaySqliteError("pairing secret envelope: iv must decode to 12 bytes");
  }
  if (ctWithTag.length < 16) {
    throw new FrameRelaySqliteError("pairing secret envelope: ciphertext too short");
  }
  ...
  try {
    ...
    throw new FrameRelaySqliteError(
      "pairing secret envelope: decrypt failed (bad key or ciphertext)",
    );
  }
}
