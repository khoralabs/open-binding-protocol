...
export const PAIRING_SECRET_ENCRYPTION_KEY_ENV = "OBP_PAIRING_SECRET_ENCRYPTION_KEY" as const;

/** 32-byte test key (64 hex chars) for unit tests and non-production dev only. */
...
export function pairingSecretKeyFromHex(hex: string): Uint8Array {
  ...
  if (!/^[0-9a-f]{64}$/i.test(trimmed)) {
    throw new FrameRelaySqliteError(
      "pairing secret encryption key must be 32-byte hex (64 characters)",
    );
  }
...
export function pairingSecretKeyFromEnv(
  ...
  if (env.NODE_ENV === "production") {
    throw new FrameRelaySqliteError(
      `${PAIRING_SECRET_ENCRYPTION_KEY_ENV} is required in production`,
    );
  }
  if (options?.allowDevFallback === false) {
    throw new FrameRelaySqliteError(`${PAIRING_SECRET_ENCRYPTION_KEY_ENV} is required`);
  }
