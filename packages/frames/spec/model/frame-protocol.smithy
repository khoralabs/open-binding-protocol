...
**Logical `Frame.body`:** At the OBP layer, **`body`** is **logical plaintext** JSON. **`sig`** is **always** computed over **`signing_bytes`** of the logical body (never over ciphertext wrappers). Transport confidentiality is **not** defined by encrypting **`body`** in this namespace.

**Transport confidentiality:** Internet hub-mediated deployments **MUST** use **`khora.obp.frame.mls`** (MLS per **RFC 9420**, outer envelope **`mls2`** per **`MlsHubEnvelope`**). Custodial deployments **MAY** use plaintext multiplex on the blob bus. Direct transports **MAY** rely on TLS only. See `packages/frames/spec/model/mls-hub-protocol.smithy`.

**Session bootstrap:** Framed objects **MAY** include multiple **`{ "init": `<SessionInit JSON>` }`** envelopes on the **same** duplex byte stream (long‑lived multiplex): each distinct **`session_id`** / **`genesis_hash`** pair starts a separate causal chain. Implementations **MUST** route each **Frame** to the unique open chain whose current tip or registered **`genesis_hash`** equals **`p_hash`**. Keys SHOULD be sorted for canonical framing. **MLS blob-hub** deployments wrap the multiplex byte stream in **`khora.obp.frame.mls#MlsHubEnvelope`** on the outer bus; inner stream uses bare **`Frame`** objects. Direct streams **without** a relay **MAY** send bare **`Frame`** objects. Relay-mediated channels carry opaque outer blobs; see relay deployment docs.

**Relay echo:** When a relay is in the path, originators MUST apply **`Frame`** effects only from **inbound channel bytes** received from the relay (including self-echo), not from the pre-relay send path. **MLS blob-hub:** hub echoes stamped **`mls2`** blobs to all peers including the sender; peers apply frame effects from peer-delivered MLS plaintext after decrypt and timing unwrap.
...
**Concurrent transport sessions:** Servers **MAY** accept **many** open streams at once (one negotiated stream per client session). **How** each logical bilateral session is backed—dedicated **`ObpPersistence`**, a shared store with partitioning, or otherwise—is **implementation-defined**; this protocol **MUST NOT** be read as requiring per-session physical isolation. Projections **MUST** satisfy **`khora.obp#ObpPersistence`** invariants in `packages/persistence/spec/model/persistence.smithy` on whatever store they use, including NBC **global canonical `NbcPortExposePolicy.max_bindings`** and **atomic** enforcement when concurrent operations mutate the **same** logical graph (see invariant **11** in `packages/persistence/spec/model/persistence.smithy` for shared vs separate store boundaries).
